				...
				if ( $_product && $_product->exists() && $cart_item['quantity'] > 0 && apply_filters( 'woocommerce_checkout_cart_item_visible', true, $cart_item, $cart_item_key ) ) {
					...
					<tr>
						<td class="product-name">
							<a href="<?php echo get_permalink($_product->id); ?>"><strong><?php echo apply_filters( 'woocommerce_cart_item_name', $_product->get_title(), $cart_item, $cart_item_key ); ?></strong></a>
							<?php if ($item_sku) { echo ' &ndash; (' . $item_sku . ')'; } ?>
						...
						<td class="product-total">
							<?php if ($prod_cart_data) { ?>
								<span class="amount"><?php echo wc_price($prod_cart_data->price); ?></span>
							<?php } else { ?>
