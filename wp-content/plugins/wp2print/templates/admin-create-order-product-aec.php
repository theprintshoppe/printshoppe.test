...
if ($product_type_matrix_types) { ?>
	<div class="print-products-area product-attributes">
						...
						<div class="matrix-type-simple" data-mtid="<?php echo $mtype_id; ?>" data-ntp="<?php echo $num_type; ?>">
							<?php if ($numbers) { ?>
								<p class="form-field">
									...
									<?php } else { ?>
										<input type="text" name="quantity" id="qty" class="quantity" value="<?php if ($product_data['quantity']) { echo $product_data['quantity']; } else { echo $numbers[0]; } ?>" onblur="matrix_calculate_price();<?php if (!$is_modify) { ?> calculate_final_price();<?php } ?>">
									<?php } ?>
								</p>
							<?php } ?>
								...
								<?php foreach($mattributes as $mattribute) {
									...
									if ($matype == 'text') { ?>
										...
										if ($aterms) {
											...
											<p class="form-field matrix-attribute-<?php echo $mtype_id; ?>-<?php echo $mattribute; ?>">
												<label><?php echo print_products_attribute_label($mattribute, $attribute_labels, $attribute_names[$mattribute]); ?>:</label>
												<select name="sattribute[<?php echo $mattribute; ?>]" class="smatrix-attr<?php echo $attr_class; ?>" data-aid="<?php echo $mattribute; ?>" onchange="matrix_calculate_price();<?php if ($mattribute == $material_attribute) { ?> calculate_final_price();<?php } ?>">
		...
		<input type="hidden" name="product_type" value="aec">
