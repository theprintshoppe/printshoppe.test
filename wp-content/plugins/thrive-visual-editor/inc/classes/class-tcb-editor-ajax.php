...
if ( ! class_exists( 'TCB_Editor_Ajax' ) ) {
	...
	class TCB_Editor_Ajax {
		...
		/**
		 * Keep the behaviour on scroll for the header in a separate meta
		 *
		 * @return array
		 */
		public function action_update_header_meta() {

			/** @var TCB_Header_Element $element */
			if ( ! ( $element = tcb_elements()->element_factory( 'header' ) ) || ! is_a( $element, 'TCB_Element_Abstract' ) ) {
				$this->error( __( 'Invalid element type', 'thrive-cb' ), 500 );
			}

			if ( ! ( $this->param( 'header_id' ) ) ) {
				$this->error( __( 'Missing header id param', 'thrive-cb' ), 500 );
			}

			$meta = array(
				'header_id'  => $this->param( 'header_id' ),
				'meta_key'   => $this->param( 'meta_key' ),
				'meta_value' => $this->param( 'meta_value' ),
			);

			$data = $element->update_meta( $meta );
