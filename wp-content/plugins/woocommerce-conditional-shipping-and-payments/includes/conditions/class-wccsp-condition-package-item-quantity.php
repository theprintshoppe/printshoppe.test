	...
	*/
	public function get_condition_resolution( $data, $args ) {
		...
		if ( $data[ 'modifier' ] === 'min' && $condition_value < $total_quantity ) {
			...
			if ( $package_count > 1 ) {
				$message = sprintf( __( 'ensure that there are no more than %s items in any affected shipping packages', 'woocommerce-conditional-shipping-and-payments' ), $condition_value );
			} else {
				$message = sprintf( __( 'ensure that there are no more than %s items in your cart', 'woocommerce-conditional-shipping-and-payments' ), $condition_value );
			}
		...
		if ( $data[ 'modifier' ] === 'max' && $condition_value > $total_quantity ) {
			...
			if ( $package_count > 1 ) {
				$message = sprintf( __( 'ensure that all affected shipping packages contain at least %s items', 'woocommerce-conditional-shipping-and-payments' ), $condition_value );
			} else {
				$message = sprintf( __( 'ensure that your cart contains at least %s items', 'woocommerce-conditional-shipping-and-payments' ), $condition_value );
			}
